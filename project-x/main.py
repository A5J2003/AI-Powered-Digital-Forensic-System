...
def run_pipeline(input_path, original_filename: str = None):
    """
    Run the full multimodal deepfake detection pipeline.

    Args:
        input_path:        Path to the (possibly temp-named) input file.
        original_filename: The real filename as uploaded by the user.
                           If None, falls back to the basename of input_path.
    """
    print("\n=========================")
    print("RUNNING FULL PIPELINE")
    print("=========================\n")

    # FIX: use original_filename so the saved result carries the real name,
    # not the UUID temp name created by the API upload handler.
    ...
    output = {
        ...
        "modalities":         results,
        "ingestion":          data,
        # FIX: store the real filename so /jobs can recover it from disk
        # even after the backend restarts (when in-memory jobs are gone)
        "original_filename":  original_filename,
        # FIX: expose case_path so api.py doesn't have to guess it from hash_db
        "_case_path":         case.base_path,
    }

    save_path = os.path.join(case.get_path("results"), "final_result.json")

    # Save to disk without the internal _case_path field
    output_for_disk = {k: v for k, v in output.items() if k != "_case_path"}
    with open(save_path, "w") as f:
        json.dump(output_for_disk, f, indent=4)
